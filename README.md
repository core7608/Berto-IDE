...
| إضافات معقدة (بها Debugger أو API خاص بـ VS Code) | ⚠️ جزئي | يتم اكتشافها وعرض حالتها بصدق، لكن التفعيل الكامل لأي إضافة معقدة يحتاج محاكاة كاملة لـ VS Code Extension Host (خارج نطاق هذا الإصدار) |
...
| بناء تلقائي ونشر على GitHub Releases | ✅ يعمل | عبر GitHub Actions |
...
- `Berto.IDE-1.0.0-win-x64.exe` (installer) + نسخة portable
- `Berto.IDE-1.0.0-mac-x64.dmg` + `.zip`
- `Berto.IDE-1.0.0-linux-x64.AppImage` + `.deb` + `.tar.gz`
...
## 🛣️ أفكار للتطوير المستقبلي (Roadmap)
